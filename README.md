...
- Transient provider failures (including overload/rate-limit/429/5xx,
  ...
  projection so the next retry reuses it; no automatic model switch occurs.
  Unknown or structural provider errors discard the projection and force one
  canonical fail-open request before projection may resume. The classification
  decision is recorded in telemetry.
